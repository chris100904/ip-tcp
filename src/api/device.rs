...
impl Device {
  ...
  pub fn start_periodic_updates(&self, device: Arc<Mutex<Device>>) {
    let mut interval = Duration::from_secs(self.rip_periodic_update_rate.unwrap()); // 5 seconds interval default
    loop {
